getSpectrum <- function(xset,mz1,mz2,lambda){
  ...
  rt.list <- round(xset@rt[[1]],2)
  spectra <- matrix(0,length(xset@sample),length(xset@rt[[1]]))
  segSize <- 0
  for (i in 1:nrow(spectra)){
    peak.i <- peakmat[peakmat[,'sample']==i,]
    peak.id <- which(peak.i[,'mz']>=mz1&peak.i[,'mz']<=mz2)
    if (length(peak.id)<1){next}
    for (j in 1:length(peak.id)){
      PIC.j <- xset@PICset$PICset[[i]]$PICs[[peak.id[j]]]
      segSize <- max(segSize,nrow(PIC.j))
      temp1 <- c(PIC.j[1,1]-0.1,0)
      temp2 <- c(PIC.j[nrow(PIC.j),1]+0.1,0)
      PIC.j <- rbind(c(0,0),temp1,PIC.j,temp2,c(max(rt.list)+5,0))
      spectra[i,] <- spectra[i,]+approx(PIC.j[,1],PIC.j[,2],rt.list)$y
    }
    # smoothing
    y <- spectra[i,]
    M <- length(y)
...
seg.rtcor <- function(xset,ref,mz1,mz2,shift,lambda){
  spectra <- getSpectrum(xset,mz1,mz2,lambda)
  segSize <- spectra$segSize
  spectra <- spectra$spectra
  peakmat <- xset@peakmat
  if (sum(spectra)==0){return(xset)}
